...
(function() {
    ...
    //排序
	function tableSort(current_directory){
		alert("tableSort ");
		clickTime = 0;
		$('table').find('thead th');
		arr = $('table').find('tbody tr');
		
		$('table tbody').html('');
		arr.sort(function (a, b){
			trs1 = $(a).find('*');
			trs2 = $(b).find('*');
			time1 = $(trs1[4]).attr('value');
			time2 = $(trs2[4]).attr('value');
			if(clickTime % 2 == 0){
				return time2 - time1;
			}else{
				return time1 - time2;
		...
		$.each(arr, function(index, value){
		  ...
		  current_directory = $('.inode-path').val();
		  $('.explorer-browse-links').click(function(current_directory) {
  ...
  function get_response_explorer(data, type) {
    return data[type] !== undefined ? data[type] : null;
  }
  ...
  function view_file_details(path, abs_path) {
    ...
    var url = '/webhdfs/v1' + abs_path + '?op=GETFILESTATUS&user.name=hadoop';
    $.get(url).done(function(data) {
      var d = get_response_explorer(data, "FileStatus");
      if (d === null) {
      ...
      var download_url = '/webhdfs/v1' + abs_path + '?op=OPEN&user.name=hadoop';
  ...
  function browse_directory(dir) {
    ...
    var url = '/webhdfs/v1' + encode_path(dir) + '?op=LISTSTATUS&user.name=hadoop';
    $.get(url, function(data) {
	  ...
	  //对d进行排序逆序
	  //对json object 进行排序
	  d.FileStatus.sort(function(a, b){
		  return (b.pathSuffix).localeCompare(a.pathSuffix)	  
	  });
			...
			function (){
				...
				if(id == "Name"){
					arr.sort(function (a, b){
						...
						trs1 = $(a).find('.explorer-browse-links').attr('inode-path');
						trs2 = $(b).find('.explorer-browse-links').attr('inode-path');
						//console.log(trs1 > trs2);
						if(clickTime % 2 == 0){
							return trs1.localeCompare(trs2);
						}
						if(clickTime % 2 == 1){
							return trs2.localeCompare(trs1);
						}
					})
				}
